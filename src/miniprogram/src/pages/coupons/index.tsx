...
export default function CouponsPage() {
  ...
  return <View className='coupons-page'>
    <View className='coupon-head'><View className='coupon-head-copy'><Text>PRIVATE BENEFITS</Text><Text>会员礼券</Text><Text>每一份礼遇，都为重要时刻而来。</Text></View><View className='coupon-seal'><IconFont name='gift' /></View></View>
    ...
    <View className='coupon-list'>
      ...
      {!loading && visible.map((coupon) => {
        const amount = formatMoney(coupon.amount_cents).replace('¥', '')
        return <View className={`coupon-card ${coupon.used || !coupon.available ? 'disabled' : ''} ${tab !== 'available' ? 'no-action' : ''}`} key={coupon.id}>
          <View className='coupon-value'>
            <View className='coupon-amount-row'><Text className={amount.length > 5 ? 'coupon-amount long' : 'coupon-amount'}>{amount}</Text><Text className='coupon-unit'>元</Text></View>
            <Text className='coupon-threshold'>满 {formatMoney(coupon.minimum_cents)} 可用</Text>
          </View>
