...
export default function FavoritesPage() {
  ...
  return <View className='favorites-page'>
    ...
    {loading ? <View className='favorites-grid'>{[0, 1, 2, 3].map((item) => <View className='favorite-skeleton' key={item} />)}</View> : items.length ? (
    ...
    ) : <View className='favorite-empty'><View className='heart-line'>♡</View><Text>尚未收藏作品</Text><Text>遇见喜欢的珠宝时，轻触心形即可保存</Text><Button onClick={() => Taro.switchTab({ url: '/pages/products/index' })}>浏览作品</Button></View>}
  </View>
}
